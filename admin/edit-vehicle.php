...
$sql="UPDATE tblvehicles SET VehiclesTitle=:vehicletitle,FkVehBrandId=:brand,VehiclesOverview=:vehicleoverview,PricePerDay=:priceperday,FkFuelTypeId=:fueltype,ModelYear=:modelyear,SeatingCapacity=:seatingcapacity,AirConditioner=:airconditioner,PowerDoorLocks=:powerdoorlocks,AntiLockBrakingSystem=:antilockbrakingsys,BrakeAssist=:brakeassist,PowerSteering=:powersteering,DriverAirbag=:driverairbag,PassengerAirbag=:passengerairbag,PowerWindows=:powerwindow,CDPlayer=:cdplayer,CentralLocking=:centrallocking,CrashSensor=:crashcensor,LeatherSeats=:leatherseats WHERE id=:id ";
...
$msg="Data updated successfully";
	...
	$msgErr="Data updated successfully";
}
...
<?php if($msgErr){?><div class="errorWrap"><strong>Error  </strong>:  <?php echo htmlentities($msg); ?> </div><?php } ?>
<?php 
$idz=intval($_GET['id']);
...
$sql ="SELECT tblvehicles.*,tblbrands.BrandName,tblbrands.id as bid, tblFuealType.id as ftid,tblFuealType.typeName FROM tblvehicles join tblbrands on tblbrands.id=tblvehicles.FkVehBrandId INNER JOIN tblFuealType ON tblFuealType.id = tblvehicles.FkFuelTypeId WHERE tblvehicles.id=:ids";

// INNER JOIN carrental.tblFuealType ON tblFuealType.id = tblvehicles.FkFuelTypeId
$query = $dbh -> prepare($sql);
$query-> bindParam(':ids', $idz, PDO::PARAM_STR);
$query->execute();
...
<option value="<?php echo htmlentities($result->ftid);?>"><?php echo htmlentities($ftypename=$result->typeName); ?> </option>


<?php $retft="SELECT * FROM tblFuealType";
$queryFT= $dbh -> prepare($retft);
//$query->bindParam(':id',$id, PDO::PARAM_STR);
$queryFT-> execute();
$resultFT = $queryFT -> fetchAll(PDO::FETCH_OBJ);
if($queryFT -> rowCount() > 0)
{
foreach($resultFT as $resultft)
{
if($resultft->typeName==$ftypename)
{
continue;
} else{
?>
<option value="<?php echo htmlentities($resultft->id);?>"><?php echo htmlentities($resultft->typeName);?></option>
<?php }}} ?>
